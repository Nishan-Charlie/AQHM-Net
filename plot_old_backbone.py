...
RESULTS_FILE = ROOT / "results_old_backbone_summary.csv"
PLOTS_DIR = ROOT / "results_organized" / "plots_old_backbone"

def load_data():
    df = pd.read_csv(RESULTS_FILE)
    df['dataset_pretty'] = df['dataset'].map(lambda x: DATASET_PRETTY.get(x, x))
    df['domain'] = df['dataset'].map(lambda x: DOMAIN.get(x, "Other"))
    
    # Handle NaNs and empty values for numeric conversion
    df['weighted_f1_mean'] = pd.to_numeric(df['weighted_f1_mean'], errors='coerce')
    df['weighted_accuracy_mean'] = pd.to_numeric(df['weighted_accuracy_mean'], errors='coerce')
    
    # Define degenerate logic like organize_results.py (< 0.20 F1)
    df['degenerate'] = df['weighted_f1_mean'].notna() & (df['weighted_f1_mean'] < 0.20)
    
    # K might be empty string or NaN for no_quantum
    df['K'] = pd.to_numeric(df['quantum_heads_K'], errors='coerce').fillna(1)
    
    # attention_encoding string parsing
    df['attention_encoding'] = df['attention_encoding'].astype(str).str.lower() == 'true'
    
    # scale defaults
    df['scale'] = df['scale'].fillna('small')
...
def plot_best_per_dataset(df):
    valid_df = df[~df['degenerate']].dropna(subset=['weighted_f1_mean'])
    if valid_df.empty: return
    
    idx = valid_df.groupby('dataset')['weighted_f1_mean'].idxmax()
    ...
    ax.set_title("Best Configurations per Dataset (Old Backbone)")
    
    save_plot(fig, "old_best_per_dataset.png")
...
def plot_scale_effect(df):
    ...
    # Pick the best accuracy for each K + Scale combination if there are multiple runs
    scale_df = scale_df.loc[scale_df.groupby(['K', 'Scale'])['weighted_accuracy_mean'].idxmax().dropna()]
    ...
    ax.set_title("Effect of Backbone Scale (CIFAR-10, Old Backbone)")
    ax.grid(True, ls='--', alpha=0.5)
    save_plot(fig, "old_effect_of_scale.png")
...
def plot_k_effect(df):
    ...
    # Best run per dataset, K combination
    k_df = k_df.loc[k_df.dropna(subset=['weighted_accuracy_mean']).groupby(['dataset', 'K'])['weighted_accuracy_mean'].idxmax()]
    ...
    ax.set_title("Effect of Parallel Circuits (small, Old Backbone)")
    ax.grid(axis='y', ls='--', alpha=0.5)
    save_plot(fig, "old_effect_of_k.png")

def main():
    if not RESULTS_FILE.exists():
        print(f"{RESULTS_FILE} not found. Please run compile_results_summary.py and filter.")
        return
